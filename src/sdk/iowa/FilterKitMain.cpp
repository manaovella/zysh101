...
#include "FFTCode.h"             // The FFT code and the windowing code (Kaiser and Sinc windows, etc.)
...
{
	ExampleFIRCall();          // Geneartes either a Rectangular Windowed FIR or a Parks McClellan FIR.
	ExampleIIRCall();          // IIR Filter coefficients
	ExampleLowPassProtoType(); // 2nd order s plane coefficients for a Butterworth, Cheby, Bessel, etc.
	ExampleFreqSampling();     // An FIR filter using frequency sampling.
	ExampleFreqSampling2();    // A 2nd example of FIR using freq sampling.
	AnalogFreqSampling();      // An analog type FIR filter using freq sampling.
}
