...
#include <spuce/filters/iir_allpass_variable_cascade.h>
namespace spuce {

class audio_equalizer {
 public:
  typedef std::complex<double> audio_data_type;
  int num_bands;
  ...
  void reset();
  std::complex<double> run(const std::complex<double>& sample);
  std::complex<double> run_fast(const std::complex<double>& sample);
  void update_levels();
  ...
  bool stage(std::complex<double> s, int i, std::complex<double>& h, std::complex<double>& l);

  std::complex<double> stage_up_down(int i, std::complex<double> s);
};
}  
